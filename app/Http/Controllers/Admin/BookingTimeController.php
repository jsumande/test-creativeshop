...
class BookingTimeController extends Controller
{
    ...
    public function update(UpdateBookingTime $request, $id){
        ...
        if($request->start_time){
            $bookingTime->start_time = Carbon::createFromFormat('h:i A', $request->start_time)->format('H:i:s');
        }
        if($request->end_time){
            $bookingTime->end_time = Carbon::createFromFormat('h:i A', $request->end_time)->format('H:i:s');
        }
